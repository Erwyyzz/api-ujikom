        ...
        try {
            $peminjaman = DB::transaction(function () use ($request) {
                ...
                foreach ($request->items as $item) {
                    // lockForUpdate mengunci baris data di database
                    // sampai transaksi ini COMMIT
    ...
    public function update(
        ...
        try {
            DB::transaction(function () use ($request, $peminjaman) {
                ...
                foreach ($request->items as $item) {
                    // Ditambahkan lockForUpdate agar konsisten aman dari
                    // race condition saat update data draft
                    $alat = Alat::lockForUpdate()->findOrFail($item['alat_id']);
        ...
        try {
            DB::transaction(function () use ($peminjaman) {
                ...
                foreach ($peminjaman->detailPinjam as $detail) {
                    // Mengunci baris alat demi validasi final sebelum
                    // stok dikurangi
